...
namespace KxEnumClassOperations
{
	...
	#define KxAllowEnumCastOp(T) template<> struct IsEnumCastOpAllowed<T>: std::true_type {}
	#define KxAllowEnumBitwiseOp(T) template<> struct IsEnumBitwiseOpAllowed<T>: std::true_type {}

	#define KxImplementEnum(T)	\
		KxAllowEnumCastOp(T);	\
		KxAllowEnumBitwiseOp(T)
}

namespace KxEnumClassOperations::Internal
{
