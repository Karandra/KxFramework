...
namespace KxSystem
{
	enum class ExitCommand: uint32_t
	{
		LogOff = EWX_LOGOFF,
	...
	enum class LockCommand: uint32_t
	{
		LockScreen,
		ScreenOn,
		ScreenOff,
	...
	enum class ProductSuite: uint32_t
	{
		None = 0,
...
{
	KxImplementEnum(KxSystem::ExitCommand);
	KxAllowEnumCastOp(KxSystem::LockCommand);

	KxAllowEnumCastOp(KxSystem::SystemType);
	KxImplementEnum(KxSystem::ProductSuite);
	KxAllowEnumCastOp(KxSystem::ProductType);
	KxAllowEnumCastOp(KxSystem::PlatformID);
}
