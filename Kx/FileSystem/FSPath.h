...
class KX_API KxFSPath final
{
	...
	private:
		wxString m_Path;
		KxFSPathNamespace m_Namespace = KxFSPathNamespace::None;
	...
	public:
		...
		{
			return m_Namespace == KxFSPathNamespace::Win32FileUNC || m_Namespace == KxFSPathNamespace::NetworkUNC;
		}
		size_t GetComponentCount() const;

		bool HasNamespace() const
		{
			return m_Namespace != KxFSPathNamespace::None;
		}
		KxFSPathNamespace GetNamespace() const
		{
			return m_Namespace;
		}
		KxFSPath& SetNamespace(KxFSPathNamespace ns)
		{
			m_Namespace = ns;
			return *this;
		}
		
		wxString GetFullPath(KxFSPathNamespace withNamespace = KxFSPathNamespace::None) const;
		...
		bool HasDrive() const;
		KxDrive GetDrive() const;
		KxFSPath& SetDrive(const KxDrive& drive);
