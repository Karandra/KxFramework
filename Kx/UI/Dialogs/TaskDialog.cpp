...
{
	wxIMPLEMENT_DYNAMIC_CLASS(KxTaskDialog, StdDialog);
	...
	void KxTaskDialog::UpdateButtonArrays(const std::vector<String>& labels, ButtonSpecArray& array)
	{
	...
	)
	{
		...
		Dialog::Create(m_Parent, id, caption, pos, size, Dialog::DefaultStyle);
	...
	StdDialogControl KxTaskDialog::AddButton(wxWindowID id, const String& label, bool prepend)
	{
		if (label.IsEmpty())
		{
	...
	StdDialogControl KxTaskDialog::AddRadioButton(wxWindowID id, const String& label)
	{
		if (label.IsEmpty())
		{
