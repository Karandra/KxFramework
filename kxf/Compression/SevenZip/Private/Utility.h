...
namespace kxf::SevenZip::Private
{
	...
	bool GetNumberOfItems(wxInputStream& stream, CompressionFormat format, size_t& itemCount, wxEvtHandler* evtHandler = nullptr);

	FileItem GetArchiveItem(const IInArchive& archive, size_t fileIndex);
	bool GetArchiveItems(wxInputStream& stream, CompressionFormat format, std::vector<FileItem>& items, wxEvtHandler* evtHandler = nullptr);

	CompressionFormat IdentifyCompressionFormat(wxInputStream& stream, const FSPath& path = {}, wxEvtHandler* evtHandler = nullptr);
